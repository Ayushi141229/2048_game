...
def draw_game_over(window):
    ...
    game_over_text = GAME_OVER_FONT.render("GAME OVER", 1, GAME_OVER_COLOR)
    restart_text = FONT.render("Press R to Restart", 1, FONT_COLOR)
...
def check_game_over(tiles):
    for tile in tiles.values():
        for delta in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            next_tile = tiles.get(f"{tile.row + delta[0]}{tile.col + delta[1]}")
...
def main(window):
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if game_state == "lost" and event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r:
                    tiles = generate_tiles()
                    game_state = "continue"
                    draw(window, tiles)

            if game_state == "continue" and event.type == pygame.KEYDOWN:
